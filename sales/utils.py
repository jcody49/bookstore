...
def get_chart(chart_type, data, **kwargs):
    ...
    # AGG is a preferred solution to write PNG files
    ...
    if chart_type == '#1':
        ...
        bars = plt.bar(range(len(data)), data['quantity'])
        plt.xlabel('Date Created')  # Explicitly set x-axis label
        plt.ylabel('Quantity')  # Explicitly set y-axis label

        # Set 8 x-axis ticks at regular intervals
        num_ticks = 8
        step = max(1, len(data) // num_ticks)
        plt.xticks(range(0, len(data), step), data['date_created'].dt.strftime('%Y-%m-%d'), rotation=45, ha='right')

        # Adjust the color of the bars for better visibility
        for bar in bars:
